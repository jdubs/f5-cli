...
class Ssl_profile:
    ...
    def create(self, parser):
        ...
        certificate_name = "/{}/{}.crt".format(args.partition,
                                               args.certificate)
        ...
        self.connection.LocalLB.ProfileClientSSL.create_v2(profile_names=[args.name],
                                                           certs=[
                                                               {'value': certificate_name, 'default_flag': False}],
        ...
        if args.chain is not None:
            if not self.__certcheck(chain_name):
                if not self.__certcheck(common_chain_name):
                    ...
                    raise Exception(
                        "Provided chain {} not in /Common".format(chain_name))
            else:
                raise Exception(
                    "Provided chain {} not in {}".format(chain_name, partition))
    ...
    def delete(self, parser):
        self.parser.add_argument(
            '--name', action='store', dest='name', required=True, help='Name of the ssl profile to delete')
        ...
        if ssl_name in self.list():
            try:
                ...
                raise Exception(
                    "Failed to delete ssl profile - this ssl profile may be associated with another profile.")
                return False
        else:
