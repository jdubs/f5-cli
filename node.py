...
class Node:
    ...
    def create(self, parser):
      ...
      """
      self.parser.add_argument('--nodes', action='store', dest='nodes', required=True, help='Comma seperate list of nodes')
      ...
      if args.nodes:
        ...
        for x in nodes_args.split(','):
          ...
          try:
            ...
            raise Exception("%s did not have a valid A record, pleaes add one and try again".format(node['name']))
        return True
      else:
    ...
    def __create(self, node, username):
      ...
      """
      try:
        self.connection.LocalLB.NodeAddressV2.create(nodes=[node['name']], addresses=[node['address']], limits=[0])
        description = ("Added by %s via cli on %s") % (username, datetime.datetime.now())
        self.connection.LocalLB.NodeAddressV2.set_description ([node['name']], [description] )
      except:
        raise Exception("Failed to create node")
      try:
        self.connection.LocalLB.NodeAddressV2.set_monitor_rule([node['name']], 
        [{'monitor_templates': ['/Common/icmp'], 'quorum': 0, 'type': 'MONITOR_RULE_TYPE_SINGLE'}]
        )
    ...
    def delete(self, parser):
      ...
      """
      self.parser.add_argument('--nodes', action='store', dest='nodes', required=True, help='Name of the nodes to delete')
      args = self.parser.parse_args()
      ...
      print node_list  
      try:
        self.connection.LocalLB.NodeAddressV2.delete_node_address(nodes=[ node_list ])
        return True
      except:
