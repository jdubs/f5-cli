...
class Device:
    ...
    def is_current_node_active(self, partition):
        ...
        try:
            ...
            status = self.connection.Management.Device.get_failover_state(
                [local_device])
            if status == ['HA_STATE_ACTIVE']:
                self.connection.System.Session.set_active_folder(
                    "/" + partition)
                return True
            else:
                return False
            ...
            raise Exception(
                "Failed to determine if {} is a master".format(device))
    ...
    def get_active_node(self):
        ...
        try:
            for device in self.connection.Management.Device.get_list():
                device_status = self.connection.Management.Device.get_failover_state(
                    [device])
    ...
    def get_config_sync_status(self):
        ...
        try:
            ...
            raise Exception(
                "Target system has pending configuration, please sync beforehand.")
