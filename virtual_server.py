...
class Virtual_server:
  ...
  def create(self, parser):
    ...
    self.parser.add_argument('--vip_name', action='store', dest='vip_name', required=True, help='Name of the vip')
    self.parser.add_argument('--vip_address', action='store', dest='vip_address', required=True, help='IP of VIP')
    self.parser.add_argument('--port', action='store', dest='port', required=True, help='Port of Vip')
    self.parser.add_argument('--protocol', action='store', dest='protocol', required=True, help="Vip's protocol, TCP, UDP")
    self.parser.add_argument('--pool', action='store', dest='pool_name', required=True, help="Vip's backend pool")
    self.parser.add_argument('--type', action='store', dest='vip_type', required=True, help="The vip's type, HTTP, Postgresql, other")
    #profiles
    self.parser.add_argument('--ssl_profile', action='store', dest='ssl_profile', required=False, help="Enable & Apply a SSL profile, requires the name of the ssl profile.")
    self.parser.add_argument('--http_redirect', action='store', dest='http_redirect', required=False, help="Create the HTTP to HTTPS redirect")
    self.parser.add_argument('--snat_profile', action='store', dest='snat_profile', required=False, help="Set the snat profile. - defaults to auto_map.")
    self.parser.add_argument('--request_logging', action='store', dest='request_logging', required=False, help="Sets the remote host syslog logging")
    
    self.parser.add_argument('--http_profile', action='store', dest='http_profile', required=False, help="Set the http profile. - defaults to default http profile")
    self.parser.add_argument('--protocol_profile', action='store', dest='protocol_profile', required=False, help="Set the tcp profile. - defaults to defaults TCP profile")
    ### Not Implmeneted for licesning reasons.
    self.parser.add_argument('--gzip_profile', action='store', dest='gzip_profile', required=False, help="Set the http compression profile. - defaults to none.")
    ...
    pool_name = self.__validate_pool_name(args.pool_name, connection, partition, parser)
    ssl_profile = args.ssl_profile    
    
    profile_list = []
    if args.protocol_profile is not None:
      profile_tcp = {"profile_context":"PROFILE_CONTEXT_TYPE_ALL", "profile_name": args.protocol_profile }
    else:
      profile_tcp = {"profile_context":"PROFILE_CONTEXT_TYPE_ALL", "profile_name":"tcp" }
    ...
    if args.ssl_profile is not None:
      ssl_profile = args.ssl_profile
      profile_ssl = {"profile_context":"PROFILE_CONTEXT_TYPE_CLIENT", "profile_name":ssl_profile}
      profile_list.append(profile_ssl)
    
    if args.request_logging is not None:
      logging_profile = args.request_logging
      request_logging = {"profile_name":"PROFILE_TYPE_REQUEST_LOGGING", "profile_name":logging_profile}
    ...
    self.connection.LocalLB.VirtualServer.create(
      definitions = [ {"name" : vip_name, "address": vip_address, "port":vip_port, "protocol":vip_protocol } ],
      wildmasks = [ NETMASK ],
      resources = [ {"type":"RESOURCE_TYPE_POOL", "default_pool_name": pool_name} ],
      profiles = [ profile_list ],
    )
    self.connection.LocalLB.VirtualServer.set_source_address_translation_automap([vip_name])
    ...
    if args.http_redirect == "true":
      ...
      if profile_ssl in profile_list: profile_list.remove(profile_ssl)
      self.connection.LocalLB.VirtualServer.create(
        definitions = [ {"name" : vip_name, "address": vip_address, "port":"80", "protocol":vip_protocol } ],
        wildmasks = [ NETMASK ],
        resources = [ {"type":"RESOURCE_TYPE_POOL", "default_pool_name": pool_name} ],
        profiles = [ profile_list ],
      )
  ...
  #except:
    ...
    if vip_name in self.list():
      self.connection.LocalLB.VirtualServer.delete_virtual_server(
        virtual_servers = [ vip_name ] )
      raise Exception("Failed to create Virtaul Server - rolling back creation of virtual server")
    else:
  ...
  def delete(self, parser):
    ...
    """
    self.parser.add_argument('--vip_name', action='store', dest='vip_name', required=True, help='Name of the vip to delete')
    args = self.parser.parse_args()
    vip_name = "/{}/{}".format(args.partition,args.vip_name)
    if vip_name in self.list():
      try:
  ...
  def set_snat_type(self, snat_profile, vip_name):
    ...
    if snat_profile in ['automap','none', 'pool']:
      if snat_profile == 'automap':
        ...
        try:
          self.connection.LocalLB.VirtualServer.set_source_address_translation_none([vip_name])
          return True
        except:
      ...
      raise Exception("Please specify the snat type, must be, automap or none. Snat pool not implmeneted")
      return False
  ...
  def __validate_pool_name(self, pool_name, connection, partition, parser):    
    ...
    if pool_name in Pool(connection, partition, parser).list():
      ...
      raise Exception("Could not find desired pool, Please create the pool first")
      return False
  ...
  def __validate_address(self, vip_address):
    ...
    """
    ip_pattern_str = re.compile('(([1-2]?[\d]{0,2}\.){1,3}([1-2]?[\d]{0,2})|any)')
    is_ip = ip_pattern_str.match(vip_address)
    if is_ip:
      return vip_address
    else:
      raise Exception("{} is not an ip address, please try again".format(vip_address))
  ...
  def __validate_type(self, vip_type):
    ...
    if vip_type in ['http','https', 'postgresql', 'other' ]:
      ...
      raise Exception("Please specify the type of VIP, http, https, postgresql, other")
      return False